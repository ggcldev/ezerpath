      ...
      <div class="h-12 shrink-0 flex items-end px-6 pb-0" data-tauri-drag-region>
        <div class="flex items-center justify-between w-full">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-[12px] text-mk-tertiary">{filteredJobs().length}</span>
          </div>
          <input
        ...
        <table class="w-full">
          ...
          <tbody>
            ...
            >
              ...
              >
                {(job) => (
                  <tr class={`border-b border-mk-separator/50 transition-colors ${job.is_new ? "bg-mk-green-dim" : "hover:bg-mk-fill"}`}>
                    <td class="text-center">
                      <button
                        class={`text-[15px] leading-none transition-colors ${
                          job.watchlisted ? "text-mk-yellow" : "text-mk-tertiary hover:text-mk-yellow"
                        }`}
                    ...
                    <td class="px-2 py-2 truncate">
                      <span class="text-[13px] font-medium text-mk-text">{job.title}</span>
                      <Show when={job.is_new}>
                        <span class="ml-1.5 px-1 py-px rounded text-[9px] font-bold bg-mk-green-dim text-mk-green">NEW</span>
                      </Show>
                    </td>
